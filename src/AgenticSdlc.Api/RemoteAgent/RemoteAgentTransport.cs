// AgenticSdlc.Api/RemoteAgent/RemoteAgentTransport.cs
// Bridges the in-process IRemoteAgentBroker to the SignalR hub: when the gateway dispatches a request,
// run it past the approval gate, then push "Execute" to connected agents. Agent replies arrive via the
// hub's CompleteRequest, which resolves the broker's pending task.
