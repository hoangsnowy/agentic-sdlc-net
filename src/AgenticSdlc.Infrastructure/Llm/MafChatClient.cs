// AgenticSdlc.Infrastructure/Llm/MafChatClient.cs
// platform-v2 (feat/sdk-maf) — ILlmClient backed by the official Azure OpenAI SDK through the
// Microsoft.Extensions.AI `IChatClient` abstraction (the substrate Microsoft Agent Framework builds on).
// Additive: selected via provider key "MAF" (or Llm:ForceProvider=MAF). The hand-rolled ClaudeClient /
// AzureOpenAiClient stay intact; this is the SDK-based path used to migrate toward MAF agents/workflows.
...
/// <summary>
/// Azure OpenAI client via the official SDK + Microsoft.Extensions.AI <see cref="IChatClient"/>.
/// Bound to the <c>Llm:AzureOpenAi</c> options; deployment name = <c>Llm:AzureOpenAi:Model</c>.
/// </summary>
public sealed class MafChatClient : ILlmClient
{
    ...
    /// <summary>Initializes the client from the LLM options.</summary>
